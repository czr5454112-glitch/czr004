"""Run Repair5G.5.5 counterfactual probes across short-probe budgets."""
...
DEFAULT_SCENARIO_DIR = "outputs/tmp/phase5p5_repair5g55_probe_budget_scenarios"
DEFAULT_SCENARIO_METADATA = "outputs/reports/phase5p5_repair5g55_probe_budget_scenario_generation.json"
DEFAULT_LOG_DIR = "outputs/logs/phase5p5_repair5g55_probe_budget_stability"
DEFAULT_JSONL = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g55_probe_budget_runs.jsonl"
DEFAULT_COMMANDS = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g55_probe_budget_commands.jsonl"
DEFAULT_UPDATES = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g55_probe_budget_ltm_updates.jsonl"
DEFAULT_PROBES = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g55_probe_budget_update_probes.jsonl"
DEFAULT_CANDIDATES = "outputs/tables/phase5p5_repair5g55_candidate_set.csv"
DEFAULT_REPORT = "outputs/reports/phase5p5_repair5g55_probe_budget_stability_run.md"
DEFAULT_SUMMARY = "outputs/reports/phase5p5_repair5g55_probe_budget_stability_run_summary.json"
...
def budget_alias(budget_ms: float) -> str:
    return f"repair5g55_budget_{int(round(budget_ms))}ms_static_context"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.skip_solver:
        run_solver_grid_g5(
            ...
            manifest="phase5p5-repair5g55-probe-budget-stability",
    ...
    observed_only = True
    try:
        validate_g55_instance_ids([int(row.get("seed", 0)) for row in probe_rows], label="Repair5G.5.5 budget probe rows")
    except SystemExit:
        observed_only = False
    summary = {
        "schema_version": "phase5p5_repair5g55_probe_budget_stability_run_summary_v1",
        "maps": maps,
        "agent_counts": agent_counts,
        "instance_ids": instance_ids,
    ...
    write_text(
        ...
        "# Phase5.5 Repair5G.5.5 Probe Budget Stability Run\n\n"
        ...
        "Budget probes are diagnostic-only and separate unstable labels from any future training-eligible pool.\n",
    )
    print(json.dumps({"probe_rows": len(probe_rows), "budgets_ms": summary["budgets_ms"]}))
    return 0 if observed_only and probe_rows else 2
