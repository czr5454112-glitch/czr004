"""Export replayable Repair5G.5.2 pre-update checkpoints on observed IDs."""
...
DEFAULT_EQUIV_SUMMARY = "outputs/reports/phase5p5_repair5g52_updatepolicy_equivalence_summary.json"
DEFAULT_POLICY_SUMMARY = "outputs/reports/phase5p5_repair5g52_policy_closure_summary.json"
DEFAULT_SCENARIO_DIR = "outputs/tmp/phase5p5_repair5g52_checkpoint_export_scenarios"
DEFAULT_SCENARIO_METADATA = "outputs/reports/phase5p5_repair5g52_checkpoint_export_scenario_generation.json"
DEFAULT_LOG_DIR = "outputs/logs/phase5p5_repair5g52_checkpoint_export"
DEFAULT_JSONL = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g52_checkpoint_export_runs.jsonl"
DEFAULT_COMMANDS = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g52_checkpoint_export_commands.jsonl"
DEFAULT_UPDATES = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g52_checkpoint_export_ltm_updates.jsonl"
DEFAULT_CHECKPOINTS = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g52_update_checkpoints.jsonl"
DEFAULT_REPORT = "outputs/reports/phase5p5_repair5g52_checkpoint_export_smoke_report.md"
DEFAULT_SUMMARY = "outputs/reports/phase5p5_repair5g52_checkpoint_export_smoke_summary.json"

METHODS = ["repair5g52_runtime_always_static_exact", "repair5g52_runtime_always_map_agent_exact"]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--equivalence-summary-json", type=Path, default=Path(DEFAULT_EQUIV_SUMMARY))
...
def gates_pass(root: Path, args: argparse.Namespace) -> tuple[bool, str]:
    equiv = load_json(resolve(args.equivalence_summary_json, root))
    policy = load_json(resolve(args.policy_summary_json, root))
    if not equiv:
        return False, "missing_updatepolicy_equivalence_summary"
    if not policy:
        return False, "missing_policy_closure_summary"
    if not equiv.get("gates", {}).get("updatepolicy_equivalence_passed"):
        return False, "updatepolicy_equivalence_failed"
    if not policy.get("gates", {}).get("policy_closure_passed"):
        return False, "policy_closure_failed"
    return True, "gates_passed"


def write_blocked(root: Path, args: argparse.Namespace, reason: str) -> int:
    summary = {
        "schema_version": "phase5p5_repair5g52_checkpoint_export_smoke_summary_v1",
        "checkpoint_rows": 0,
    ...
    write_text(
        ...
        "# Phase5.5 Repair5G.5.2 Checkpoint Export Smoke\n\n"
        f"Blocked: `{reason}`.\n\n"
        "Replayable checkpoints are not exported for learning until UpdatePolicy equivalence and policy closure pass.\n",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.skip_solver:
        run_solver_grid_g5(
            ...
            manifest="phase5p5-repair5g52-checkpoint-export",
