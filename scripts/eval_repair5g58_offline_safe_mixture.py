"""Evaluate optional Repair5G.5.8 offline safe-mixture model."""
...
DEFAULT_MODEL = "artifacts/models/laur_ltm/repair5g58_offline_safe_mixture/model_spec.json"
...
DEFAULT_EVAL_CSV = "outputs/tables/phase5p5_repair5g58_offline_safe_mixture_eval.csv"
DEFAULT_REPORT = "outputs/reports/phase5p5_repair5g58_offline_safe_mixture_eval.md"
DEFAULT_SUMMARY = "outputs/reports/phase5p5_repair5g58_offline_safe_mixture_eval_summary.json"
...
def write_no_model(root: Path, args: argparse.Namespace, decision: str) -> int:
    eval_csv = resolve(args.eval_csv, root)
    write_csv_rows(
        eval_csv,
        [],
        fields=[
    ...
    summary = {
        "schema_version": "phase5p5_repair5g58_offline_safe_mixture_eval_summary_v1",
    ...
    write_text(resolve(args.report, root), f"# Phase5.5 Repair5G.5.8 Offline Safe Mixture Eval\n\nEvaluation skipped: `{decision}`.\n")
    print(json.dumps({"decision": decision, "offline_eval_run": False}))
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not model_path.exists():
        return write_no_model(root, args, "blocked_no_trained_model")
    model = load_json(model_path)
    if not model:
        return write_no_model(root, args, "blocked_empty_model")
    ...
    classes = list(model.get("classes", []))
    dev_rows = [row for row in features if int(float(row.get("seed") or 0)) > 150]
    eval_rows = []
    ...
    observed_dev_only = validate_observed_rows(dev_rows, label="Repair5G.5.8 offline eval dev")
    gates = {
    ...
    summary = {
        ...
        else "offline_g6_safe_mixture_failed_continue_labels_or_candidate_space",
        "phase5p5_allowed": False,
        "phase6_allowed": False,
        "aaai_ready": False,
        "runtime_claim_allowed": False,
    ...
    write_text(
        ...
        "# Phase5.5 Repair5G.5.8 Offline Safe Mixture Eval\n\n"
        ...
        f"- beats_majority_expert: `{gates['beats_majority_expert']}`\n"
    ...
    return 0 if eval_rows and observed_dev_only else 2
