"""Run Repair5G.3 time-budget and LTM-iteration stress diagnostics."""
...
DEFAULT_LOG_DIR = "outputs/logs/phase5p5_repair5g3_time_iteration_stress"
DEFAULT_JSONL = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g3_time_iteration_stress.jsonl"
DEFAULT_COMMANDS = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g3_time_iteration_stress_commands.jsonl"
DEFAULT_UPDATES = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g3_time_iteration_stress_ltm_updates.jsonl"
DEFAULT_PAIRED = "outputs/tables/phase5p5_repair5g3_time_iteration_stress_paired.csv"
DEFAULT_SUMMARY_CSV = "outputs/tables/phase5p5_repair5g3_time_iteration_stress_summary.csv"
DEFAULT_BY_BUDGET = "outputs/tables/phase5p5_repair5g3_time_iteration_stress_by_budget.csv"
DEFAULT_BY_ITER = "outputs/tables/phase5p5_repair5g3_time_iteration_stress_by_iteration.csv"
DEFAULT_REPORT = "outputs/reports/phase5p5_repair5g3_time_iteration_stress_report.md"
DEFAULT_SUMMARY = "outputs/reports/phase5p5_repair5g3_time_iteration_stress_summary.json"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--instance-ids", nargs="+", type=int, default=list(range(66, 86)))
    parser.add_argument("--time-limit-sec", nargs="+", type=float, default=[1.0, 3.0, 5.0])
    ...
    *,
    ...
    if not args.skip_solver:
        run_solver_grid(
            ...
            manifest="phase5p5-repair5g3-time-iteration-stress",
            status_json=output_dir / f"{token}.status.json",
        )
    rows = [row for row in read_jsonl(raw_jsonl) if str(row.get("method")) in set(actual_methods)]
...
def write_stress_report(path: Path, summary: dict[str, Any]) -> None:
    write_method_report(path, title="Phase5.5 Repair5G.3 Time Iteration Stress Report", summary=summary)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        raise SystemExit("broader validation protocol gates failed; stress run is blocked")
    ...
    if args.overwrite:
        for path in [output_jsonl, command_log, resolve(args.update_log, root)]:
            if path.exists():
                path.unlink()
    ...
    for time_budget in [float(value) for value in args.time_limit_sec]:
        for iteration_budget in [int(value) for value in args.ltm_max_iterations]:
            ...
            rows = [
                ...
                *synthesize_seeded_diagnostics(
                    rows,
                    synthetic_names=["repair5g3_shuffled_flow_shield_diagnostic_seed0"],
    ...
    write_update_summary(resolve(args.update_log, root), all_rows)
    ...
    selected_by_budget = {
        str(row["time_budget_sec"]): row
        for row in by_budget
        if row.get("method") == SELECTED
    }
    selected_by_iter = {
        str(row["ltm_iteration_budget"]): row
        for row in by_iteration
        if row.get("method") == SELECTED
    }
    gates = {
    ...
    interpretation = (
        "1s is treated as noisy; 3s should reproduce sign, 5s should not reverse sign, and 8 iterations should not introduce broad harm."
    )
    summary = {
        "schema_version": "phase5p5_repair5g3_time_iteration_stress_summary_v1",
        "created_at": now_iso(),
    ...
    write_stress_report(resolve(args.report, root), summary)
