"""Attempt Repair5G.5.2 counterfactual UpdateLTM probes from checkpoints."""
...
DEFAULT_REPLAY_SUMMARY = "outputs/reports/phase5p5_repair5g52_checkpoint_replayability_summary.json"
DEFAULT_CHECKPOINTS = "outputs/logs/phase5p5_repair5g52_checkpoint_export/phase5p5_repair5g52_update_checkpoints.jsonl"
...
DEFAULT_REPORT = "outputs/reports/phase5p5_repair5g52_counterfactual_label_report.md"
DEFAULT_SUMMARY = "outputs/reports/phase5p5_repair5g52_counterfactual_label_summary.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    root = repo_root()
    replay_summary = json.loads(resolve(args.replayability_summary_json, root).read_text(encoding="utf-8")) if resolve(args.replayability_summary_json, root).exists() else {}
    ...
    if not replay_ok and blocked_reason:
        ...
        gap = "checkpoint export exists only as JSONL; no checkpoint replay/probe CLI is available yet"
    ...
    for index, checkpoint in enumerate(checkpoints):
        ...
        for candidate in CANDIDATES:
            # The checkpoint exporter records replayable state, but this phase
            # still lacks a C++ CLI that imports that state and invokes
            # run_one_shot_update_probe for each candidate. Keep the gap hard.
            rows.append(
                {
        ...
        oracle_rows.append(
            {
                "context_id": context_id,
                "candidate_coverage_complete": len(available_candidates) == len(CANDIDATES),
                "oracle_candidate_id": "",
    ...
    summary = {
        "schema_version": "phase5p5_repair5g52_counterfactual_label_summary_v1",
        ...
        "gates": gates,
        "gap": gap,
        "decision": "continue_counterfactual_label_collection" if gates["counterfactual_labels_passed"] else "counterfactual_labels_unavailable",
    ...
    write_text(
        ...
        "# Phase5.5 Repair5G.5.2 Counterfactual UpdateLTM Labels\n\n"
