"""Analyze Repair5G dual-channel LTM development probe oracle headroom."""
...
DEFAULT_REPORT = "outputs/reports/phase5p5_repair5g_dual_channel_oracle_report.md"
DEFAULT_SUMMARY = "outputs/reports/phase5p5_repair5g_dual_channel_oracle_summary.json"
DEFAULT_RANKING = "outputs/tables/phase5p5_repair5g_dual_channel_candidate_ranking.csv"
DEFAULT_ORACLE_BY_CASE = "outputs/tables/phase5p5_repair5g_dual_channel_oracle_by_case.csv"
DEFAULT_GROUP_BEST = "outputs/tables/phase5p5_repair5g_dual_channel_group_best_candidates.csv"
DEFAULT_DECISION = "outputs/reports/phase5p5_repair5g_dual_channel_decision.md"

ORACLE_METHOD = "repair5g_dual_channel_oracle_static_proxy"
...
def dual_candidate_methods() -> dict[str, str]:
    return {
...
def synthesize_oracle_rows(rows: list[dict[str, Any]], candidate_methods: set[str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def bootstrap_ci(values: list[float], *, samples: int = 2000, seed: int = 20260602) -> dict[str, float | None]:
    clean = [value for value in values if math.isfinite(value)]
    if not clean:
        return {"mean": None, "ci_low": None, "ci_high": None, "samples": 0}
...
def component_rows(ranking: list[dict[str, Any]], components: dict[str, str]) -> list[dict[str, Any]]:
...
def decide(summary: dict[str, Any]) -> str:
    ...
    if not gates["dual_additive_parity_exact"] or not gates["cost_bounds_respected"]:
        return "implementation_not_trusted"
    if oracle.get("better", 0) > oracle.get("worse", 0) and finite(oracle.get("mean_delta_ratio_vs_ltm")) < -0.01:
        return "dual_channel_has_strong_headroom"
    if oracle.get("better", 0) > oracle.get("worse", 0) and finite(oracle.get("mean_delta_ratio_vs_ltm")) < 0.0:
        return "mixed"
    return "dual_channel_no_headroom"


def write_report(path: Path, summary: dict[str, Any], ranking: list[dict[str, Any]], components: list[dict[str, Any]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("# Phase5.5 Repair5G Dual-Channel Oracle Report\n\n")
        handle.write("This is development/diagnostic-only evidence on IDs 26..45. It does not permit Phase5.5 or Phase6.\n\n")
        handle.write("## Gates\n\n")
        ...
        handle.write("| rank | method | better | equal | worse | mean delta ratio vs LTM |\n")
        handle.write("|---:|---|---:|---:|---:|---:|\n")
        for index, row in enumerate(ranking[:20], start=1):
            handle.write(
                f"| {index} | `{row['method']}` | {row.get('better')} | {row.get('equal')} | "
...
def write_decision(path: Path, summary: dict[str, Any]) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("# Phase5.5 Repair5G Dual-Channel Decision\n\n")
        handle.write(f"Decision: `{decision}`\n\n")
        handle.write("This decision is diagnostic-only. `phase5p5_allowed=false` and `phase6_allowed=false` remain mandatory.\n\n")
        if decision == "dual_channel_has_strong_headroom":
            handle.write("Plan G1 support/validation selector work, reserving IDs 46..65 or later for untouched final validation.\n")
        elif decision == "mixed":
            handle.write("Design a narrower follow-up around the best goal-gated or C+F components before any support/validation selector.\n")
        elif decision == "implementation_not_trusted":
            handle.write("Fix parity, cost-bound, or update-log evidence before running larger experiments.\n")
        else:
            handle.write("Stop the broad dual-channel G0 lattice and return to Repair5F selector protocol or richer features.\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    components = dual_candidate_methods()
    ...
    ranking = [
        ...
        for method in stats
        if method in candidate_methods or method in {ORACLE_METHOD, "repair5g_random_dual_candidate_diagnostic", "repair5g_shuffled_goal_progress_diagnostic"}
    ]
    ranking.sort(
        key=lambda row: (
            finite(row.get("mean_delta_ratio_vs_ltm"), float("inf")),
    ...
    component_summary = component_rows(ranking, components)
    oracle_stats = stats.get(ORACLE_METHOD, {"rows": 0, "better": 0, "equal": 0, "worse": 0, "mean_delta_ratio_vs_ltm": None})
    oracle_deltas = [float(row["delta_ratio"]) for row in paired if row.get("contender_method") == ORACLE_METHOD and row.get("delta_ratio") is not None]
    gates = {
        "dual_additive_parity_exact": stats.get("repair5g_dual_additive_parity", {}).get("mean_delta_ratio_vs_ltm") == 0,
        "cost_bounds_respected": all(bool(row.get("repair5g_cost_bounds_respected", True)) for row in rows if str(row.get("method", "")).startswith("repair5g_dual_")),
    ...
    summary = {
        "schema_version": "phase5p5_repair5g_dual_channel_oracle_summary_v1",
    ...
    write_report(resolve(args.report, root), summary, ranking, component_summary)
