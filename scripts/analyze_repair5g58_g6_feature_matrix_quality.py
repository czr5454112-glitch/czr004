"""Analyze Repair5G.5.8 G6 feature matrix quality."""
...
DEFAULT_TARGET_SUMMARY = "outputs/reports/phase5p5_repair5g58_confidence_weighted_targets_summary.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    forbidden_audit = forbidden_columns(audit_plus_features)
    observed_ok = validate_observed_rows(perf_rows + audit_rows, label="Repair5G.5.8 feature matrix quality")
    training_eligible_contexts = int(target_summary.get("training_eligible_contexts", 0) or 0)
    gates = {
    ...
    summary = {
        ...
        "forbidden_audit_plus_feature_columns": forbidden_audit,
    ...
    write_text(
        ...
        "`audit_plus_perf` remains diagnostic-only. Runtime-performance claims may only use `perf_safe_only`.\n",
    )
