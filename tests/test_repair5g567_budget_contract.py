...
def test_agent_tier_primary_budget_contract_uses_tiered_large_agent_budgets() -> None:
    ...
    for tier in [1000, 1500, 2000, 2500]:
        budget_ms, base_sec, ltm_iters, role = g567.budget_profile_for_agent_tier(tier, 0)
    ...
    budget_ms, base_sec, ltm_iters, role = g567.budget_profile_for_agent_tier(3000, 0)
    assert budget_ms == 60000
    assert base_sec == 60.0
    assert ltm_iters == 12
    assert role == "agent3000_60s_primary_exact"
    assert g567.process_hard_timeout_for_internal_budget(base_sec) == 120.0
...
def test_nonprimary_purpose_does_not_override_agent_tier_primary_contract() -> None:
    for tier, expected in [
        ...
        (1500, (40000, 40.0, 12, "large_agent_40s_primary_exact")),
        (3000, (60000, 60.0, 12, "agent3000_60s_primary_exact")),
    ]:
...
def test_plan_rows_keep_same_agent3000_60s_budget_for_all_methods() -> None:
    ctx = _context(agents=3000, base_sec=60.0, budget_ms=60000)
    ...
    assert {row["solver_internal_time_limit_sec"] for row in plan} == {60.0}
    assert {row["process_hard_timeout_sec"] for row in plan} == {120.0}
    assert {row["budget_role"] for row in plan} == {"agent3000_60s_primary_exact"}
...
def test_response_acquisition_metadata_is_preserved_in_plan_and_registry() -> None:
    ...
    theta_row = {
        ...
        "primary_exact_solver_internal_time_limit_sec": 60.0,
        "primary_exact_process_hard_timeout_sec": 120.0,
        **theta,
    }
    ...
    for row in [actor_plan, actor_registry]:
        ...
        assert row["primary_exact_solver_internal_time_limit_sec"] == 60.0
        assert row["primary_exact_process_hard_timeout_sec"] == 120.0


def test_plan_generation_fails_closed_without_explicit_hard_timeout() -> None:
    ctx = _context(agents=3000, base_sec=60.0, budget_ms=60000, hard_timeout_sec=0.0)
...
def test_direct_exact_resume_rejects_stale_large_agent_30s_result() -> None:
    plan = {
        ...
        "solver_internal_time_limit_sec": 60.0,
...
def test_primary_30s_response_generation_uses_selected_candidate_not_full_lattice() -> None:
    ...
    assert rows[0]["primary_exact_solver_internal_time_limit_sec"] == 60.0
    assert rows[0]["primary_exact_process_hard_timeout_sec"] == 120.0
...
def test_gate3b_selected_30s_acquisition_can_hit_response_row_target() -> None:
    contexts = [
        ...
        _context(agents=3000, base_sec=60.0, budget_ms=60000),
    ]
    raw_rows = [
        {
            "context_id": ctx.dataset_row_id,
