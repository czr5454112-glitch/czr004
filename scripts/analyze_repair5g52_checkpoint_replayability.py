"""Analyze Repair5G.5.2 exported checkpoint replayability."""
...
DEFAULT_EXPORT_SUMMARY = "outputs/reports/phase5p5_repair5g52_checkpoint_export_smoke_summary.json"
DEFAULT_REPORT = "outputs/reports/phase5p5_repair5g52_checkpoint_replayability_report.md"
DEFAULT_SUMMARY = "outputs/reports/phase5p5_repair5g52_checkpoint_replayability_summary.json"
DEFAULT_MANIFEST = "outputs/tables/phase5p5_repair5g52_checkpoint_manifest.csv"
...
def main(argv: list[str] | None = None) -> int:
    ...
    default_checkpoint_path = resolve(Path(DEFAULT_CHECKPOINTS), root)
    export_summary = (
        load_json(resolve(args.checkpoint_export_summary_json, root))
    ...
    if export_summary and not export_summary.get("checkpoint_export_passed"):
        ...
        summary = {
            "schema_version": "phase5p5_repair5g52_checkpoint_replayability_summary_v1",
            "checkpoint_rows": 0,
        ...
        write_text(
            ...
            "# Phase5.5 Repair5G.5.2 Checkpoint Replayability\n\n"
            f"Blocked: `{reason}`.\n\n"
            "Replayability is not evaluated until checkpoint export is allowed by the UpdatePolicy equivalence gate.\n",
        )
    ...
    traffic_before_present = checkpoint_rows > 0 and all(
        isinstance(row.get("traffic_before_edges"), list) for row in rows
    )
    ...
    gates["checkpoint_replayability_passed"] = all(gates[key] for key in [
