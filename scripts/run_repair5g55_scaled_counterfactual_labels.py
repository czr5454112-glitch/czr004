"""Run Repair5G.5.5 scaled same-context counterfactual label collection."""
...
DEFAULT_SCENARIO_DIR = "outputs/tmp/phase5p5_repair5g55_scaled_counterfactual_scenarios"
DEFAULT_SCENARIO_METADATA = "outputs/reports/phase5p5_repair5g55_scaled_counterfactual_scenario_generation.json"
DEFAULT_LOG_DIR = "outputs/logs/phase5p5_repair5g55_scaled_counterfactual_labels"
DEFAULT_JSONL = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g55_scaled_counterfactual_runs.jsonl"
DEFAULT_COMMANDS = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g55_scaled_counterfactual_commands.jsonl"
DEFAULT_UPDATES = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g55_scaled_counterfactual_ltm_updates.jsonl"
DEFAULT_PROBES = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g55_counterfactual_update_probes.jsonl"
DEFAULT_CHECKPOINTS = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g55_update_checkpoints.jsonl"
DEFAULT_CANDIDATES = "outputs/tables/phase5p5_repair5g55_candidate_set.csv"
DEFAULT_REPORT = "outputs/reports/phase5p5_repair5g55_scaled_counterfactual_run.md"
DEFAULT_SUMMARY = "outputs/reports/phase5p5_repair5g55_scaled_counterfactual_run_summary.json"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--maps", nargs="+", default=G55_MAPS)
    parser.add_argument("--agent-counts", nargs="+", type=int, default=G55_AGENT_COUNTS)
    parser.add_argument("--instance-ids", nargs="+", default=[f"{G55_DEFAULT_INSTANCE_IDS[0]}..{G55_DEFAULT_INSTANCE_IDS[-1]}"])
...
def method_specs(selector_spec: Path, probe_jsonl: Path, checkpoint_jsonl: Path, args: argparse.Namespace, candidates: list[str]) -> list[MethodSpec]:
    ...
    return [MethodSpec(G55_STATIC_CONTEXT_METHOD, G55_CONTEXT_ALIAS, extra)]


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv)
    root = repo_root()
    instance_ids = validate_g55_instance_ids(args.instance_ids)
    ...
    if not args.skip_solver:
        run_solver_grid_g5(
            ...
            manifest="phase5p5-repair5g55-scaled-counterfactual-labels",
    ...
    contexts = group_by_context(probe_rows)
    observed_only = True
    try:
        validate_g55_instance_ids([int(row.get("seed", 0)) for row in probe_rows], label="Repair5G.5.5 probe rows")
    except SystemExit:
        observed_only = False
    summary = {
        "schema_version": "phase5p5_repair5g55_scaled_counterfactual_run_summary_v1",
        "maps": maps,
        "agent_counts": agent_counts,
        "instance_ids": instance_ids,
        "time_limit_sec": float(args.time_limit_sec),
        ...
        "context_minimum_smoke": G55_CONTEXT_MINIMUM_SMOKE,
        "context_target": G55_CONTEXT_TARGET,
    ...
    write_text(
        ...
        "# Phase5.5 Repair5G.5.5 Scaled Counterfactual Run\n\n"
        f"- maps: `{maps}`\n"
        f"- agent_counts: `{agent_counts}`\n"
        f"- instance_ids: `{instance_ids}`\n"
        f"- short_budget_ms: `{float(args.short_budget_ms)}`\n"
        ...
        "This run only collects observed-ID diagnostic labels. It does not train G6 or claim Phase5.5/Phase6 readiness.\n",
    )
    print(json.dumps({"context_count": len(contexts), "probe_rows": len(probe_rows), "checkpoint_rows": len(checkpoint_rows)}))
