"""Run Repair5G.5.6 warehouse-only probe-budget sentinel."""
...
from repair5g56_common import G56_AGENT_COUNTS, group_by_context, repo_root, resolve, validate_g55_instance_ids, validate_observed_rows, write_json, write_text  # noqa: E402


DEFAULT_LOG_DIR = "outputs/logs/phase5p5_repair5g56_warehouse_probe_budget_sentinel"
DEFAULT_SCENARIO_DIR = "outputs/tmp/phase5p5_repair5g56_warehouse_probe_budget_scenarios"
DEFAULT_SCENARIO_METADATA = "outputs/reports/phase5p5_repair5g56_warehouse_probe_budget_scenario_generation.json"
DEFAULT_JSONL = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g56_warehouse_probe_budget_runs.jsonl"
DEFAULT_COMMANDS = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g56_warehouse_probe_budget_commands.jsonl"
DEFAULT_UPDATES = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g56_warehouse_probe_budget_ltm_updates.jsonl"
DEFAULT_PROBES = f"{DEFAULT_LOG_DIR}/phase5p5_repair5g56_warehouse_probe_budget_update_probes.jsonl"
DEFAULT_CANDIDATES = "outputs/tables/phase5p5_repair5g55_candidate_set.csv"
DEFAULT_REPORT = "outputs/reports/phase5p5_repair5g56_warehouse_probe_budget_sentinel_run.md"
DEFAULT_SUMMARY = "outputs/reports/phase5p5_repair5g56_warehouse_probe_budget_sentinel_run_summary.json"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--budgets-ms", nargs="+", type=float, default=[1000.0, 2000.0, 5000.0])
...
def budget_alias(budget_ms: float) -> str:
    return f"repair5g56_warehouse_budget_{int(round(budget_ms))}ms"
...
def main(argv: list[str] | None = None) -> int:
    ...
    root = repo_root()
    instance_ids = validate_g55_instance_ids(args.instance_ids, label="Repair5G.5.6 warehouse sentinel")
    ...
    if not args.skip_solver:
        run_solver_grid_g5(
            ...
            manifest="phase5p5-repair5g56-warehouse-probe-budget-sentinel",
            status_json=output_jsonl.with_name(output_jsonl.stem + "_status.json"),
        )
    rows = read_jsonl(probe_jsonl)
    observed_only = validate_observed_rows(rows, label="Repair5G.5.6 warehouse sentinel probes")
    summary = {
        "schema_version": "phase5p5_repair5g56_warehouse_probe_budget_sentinel_run_summary_v1",
        "maps": maps,
        "agent_counts": agent_counts,
        "instance_ids": instance_ids,
    ...
    write_text(
        ...
        "# Phase5.5 Repair5G.5.6 Warehouse Probe-Budget Sentinel Run\n\n"
        ...
        "Warehouse sentinels classify probe failure modes; they are not a map cherry-pick filter.\n",
    )
