...
## 2026-06-16 - G5.55 strategic update: fixed-global staticflow baseline promoted, learning claims still closed

G5.55 repaired the G5.54 pair-schema and shortlist analysis path. The G5.54 raw rows did contain fixed-global candidates that satisfied the intended zero-regression and quality gates, but the analysis code read the wrong success field and blocked them before validation. Corrected Stage2 recomputation produced `82` validation-shortlist candidates.
